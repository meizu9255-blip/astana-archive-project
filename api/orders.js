...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (!id || !fullName || !iin || !email || !phone || !type || !query || !date || !status) {
      return res.status(400).json({ error: 'Missing required fields' });
    }
    if (!/^\d{12}$/.test(iin)) {
      return res.status(400).json({ error: 'IIN must be exactly 12 digits' });
    }
    if (fullName.length > 255 || email.length > 255 || phone.length > 50 || type.length > 255) {
      return res.status(400).json({ error: 'Field length exceeded' });
    }
    if (query.length > 1000) {
      return res.status(400).json({ error: 'Query length exceeded maximum allowed 1000 characters' });
    }
