...
const OfficialCertificateTemplate = forwardRef(({ order, adminText }, ref) => {
  ...
  return (
    <div 
      ref={ref}
      // A4 dimensions at 96 DPI: 794px by 1123px
    ...
    >
      ...
      <div className="flex justify-between items-start mt-6 text-[15px] leading-relaxed">
        <div>
          <div><strong>Күні / Дата:</strong> {today}</div>
          <div><strong>Шығыс № / Исх. №:</strong> REQ-{order.id}</div>
        </div>
        <div className="text-right max-w-[50%]">
          <div><strong>Кімге / Кому:</strong></div>
          <div className="font-semibold">{order.full_name}</div>
          {order.type && (
            <div className="mt-2 text-sm italic">Сұраныс түрі / Тип запроса:<br/>{order.type}</div>
          )}
        </div>
      </div>
