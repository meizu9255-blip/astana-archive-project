...
export default function Contacts() {
  ...
  const [activeCenter, setActiveCenter] = useState([51.1466, 71.4465]);
  ...
  const branches = [
    ...
    },
    {
      ...
      name:
        lang === "ru"
          ? "Служба научно-исследовательской работы"
          : "Ғылыми-зерттеу жұмысы қызметі",
          ...
          <div className="bg-white dark:bg-slate-800 p-8 md:p-10 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 transition-colors">
            ...
            <div className="space-y-6">
              <div className="flex items-start space-x-4">
                <div className="bg-brand-blue/10 dark:bg-slate-700 p-3 rounded-xl shrink-0">
                  <Phone className="text-brand-blue dark:text-brand-cyan w-6 h-6" />
                </div>
                <div>
              ...
              <div className="flex items-start space-x-4">
                <div className="bg-brand-blue/10 dark:bg-slate-700 p-3 rounded-xl shrink-0">
                  <Mail className="text-brand-blue dark:text-brand-cyan w-6 h-6" />
                </div>
                <div>
                  <h4 className="font-bold text-slate-800 dark:text-slate-200">
                    {c.emailLabel}
              ...
              <div className="flex items-start space-x-4">
                <div className="bg-brand-gold/20 dark:bg-slate-700 p-3 rounded-xl shrink-0">
                  <Clock className="text-brand-dark dark:text-brand-gold w-6 h-6" />
                </div>
                <div>
                  <h4 className="font-bold text-slate-800 dark:text-slate-200">
                    {c.scheduleLabel}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-1 space-y-4">
              {branches.map((branch) => (
                <div
                  key={branch.id}
                  className="p-5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50 hover:border-brand-blue dark:hover:border-brand-cyan transition-colors"
                >
