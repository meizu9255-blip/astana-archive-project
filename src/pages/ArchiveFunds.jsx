...
export default function ArchiveFunds() {
  ...
  const getCategoryStyle = (categoryRu) => {
    if (!categoryRu)
      return {
        icon: FolderOpen,
        gradient:
          "from-slate-100 to-gray-200 dark:from-slate-800 dark:to-gray-700",
        iconColor: "text-slate-600 dark:text-slate-400",
      };
    if (categoryRu.includes("Дореволюционный"))
      return {
        icon: BookOpen,
        gradient:
    ...
    if (categoryRu.includes("Советский"))
      return {
        icon: FileText,
        gradient:
          "from-red-50 to-rose-100 dark:from-red-900/20 dark:to-rose-900/20",
        iconColor: "text-red-600 dark:text-red-500",
      };
    if (categoryRu.includes("Современный"))
      return {
        icon: Building2,
        gradient:
    ...
    if (categoryRu.includes("Фото"))
      return {
        icon: Camera,
        gradient:
          "from-purple-50 to-fuchsia-100 dark:from-purple-900/20 dark:to-fuchsia-900/20",
  ...
  const filteredFunds = fundsData.filter((fund) => {
    ...
    const matchesSearch =
      title.includes(searchLower) || shifr.includes(searchLower);
  ...
  return (
    <div className="py-12 bg-slate-50 dark:bg-slate-900 min-h-screen relative transition-colors duration-300">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredFunds.map((fund) => (
              <div
                key={fund.id}
                className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl cursor-pointer group flex flex-col h-full relative overflow-hidden"
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${getCategoryStyle(fund.category_ru).gradient} opacity-50 dark:opacity-20 pointer-events-none`}
                ></div>
                ...
                <div className="relative z-10 flex justify-between items-start mb-6">
                  <div className="bg-white dark:bg-slate-800 p-3 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 group-hover:scale-110 transition-transform duration-300">
                    {React.createElement(
                      getCategoryStyle(fund.category_ru).icon,
                      {
                        className: `w-6 h-6 ${getCategoryStyle(fund.category_ru).iconColor}`,
                      },
                    )}
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-white/50 dark:bg-slate-800/50 px-2 py-1 rounded-md backdrop-blur-sm">
                      {fund.period}
                    </span>
                    <span
                      className={`text-xs px-2 py-1 rounded-md font-bold shadow-sm
                      ${
                        fund[`status_${lang}`] === "Доступен" ||
                        fund[`status_${lang}`] === "Қолжетімді"
          ...
          <div className="bg-white dark:bg-slate-800 p-12 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center text-center transition-colors duration-300">
            ...
            <p className="text-slate-500 dark:text-slate-400 max-w-md">
              {f.notFoundDesc
                ...
                .replace("{category}", f.categories[activeCategoryIndex])}
            </p>
            <button
              onClick={() => {
                setSearchTerm("");
      ...
      <DetailsModal
        ...
        meta={
          selectedFund
            ? `${f.modalShifr}: ${selectedFund.code} | ${f.modalCategory}: ${selectedFund[`category_${lang}`]} | ${f.modalPeriod}: ${selectedFund.period}`
            : ""
        }
