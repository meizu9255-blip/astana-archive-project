...
export default function Status() {
  ...
  const activeLineClass =
    currentDbStatus === "Готово к выдаче"
      ? "w-full"
      : currentDbStatus === "В обработке"
        ? "w-1/2"
        : "w-0";
  ...
  const t = {
    ...
    desc:
      lang === "ru"
        ? "Введите номер вашей заявки (например, REQ-123), чтобы узнать текущий этап ее обработки в архиве."
        : "Өтінішіңіздің (мысалы, REQ-123) өңделуінің ағымдағы кезеңін білу үшін оның нөмірін енгізіңіз.",
    ...
    resultTitle:
      lang === "ru" ? "Результат по заявке: " : "Өтініш бойынша нәтиже: ",
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <form
            ...
            className="flex flex-col md:flex-row gap-4 relative md:items-start"
          >
          ...
          >
            ...
            {currentDbStatus === "Отклонено" ? (
              <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 p-6 rounded-xl text-center font-bold">
                Ваша заявка отклонена. Пожалуйста, обратитесь в архив по номеру
                телефона для уточнения деталей.
              </div>
            ) : (
              <div className="relative">
                ...
                <div
                  className={`hidden md:block absolute top-1/2 left-0 h-1 bg-brand-gold -translate-y-1/2 rounded-full z-0 transition-all duration-700 ${activeLineClass}`}
                ></div>
                ...
                <div className="flex flex-col md:flex-row justify-between relative z-10 gap-8 md:gap-0">
                  {steps.map((step, index) => (
                    <div
                      key={step.id}
                      className="flex md:flex-col items-center group relative"
                    >
                      {index !== steps.length - 1 && (
                        <div
                          className={`md:hidden absolute left-6 top-14 bottom-0 w-1 ${step.status === "done" ? "bg-brand-gold" : "bg-slate-200 dark:bg-slate-700"} z-0`}
                          style={{ height: "calc(100% + 2rem)" }}
                        ></div>
                      )}

                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 border-4 relative z-10 transition-transform duration-300 group-hover:scale-110 
                      ...
                      <div className="ml-4 md:ml-0 md:mt-4 text-left md:text-center">
                        <div
                          className={`font-bold text-lg md:text-base ${step.status === "pending" ? "text-slate-400 dark:text-slate-500" : "text-slate-800 dark:text-slate-100"}`}
                        >
                          {step.title}
                        </div>
                        <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-wider font-semibold">
                          ...
                          {step.status === "current" &&
                            (lang === "ru" ? "Текущий этап" : "Ағымдағы кезең")}
                          {step.status === "pending" &&
                            (lang === "ru" ? "Ожидает" : "Күтілуде")}
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              statusResult.document_url && (
                <div className="mt-10 pt-8 border-t border-slate-200 dark:border-slate-700 flex flex-col items-center animate-fade-in-up">
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-4 text-center">
                    {lang === "ru"
                      ? "Ваша архивная справка готова. Вы можете скачать ее в электронном формате (PDF)."
                      : "Сіздің мұрағаттық анықтамаңыз дайын. Оны электронды форматта (PDF) жүктей аласыз."}
                  </p>
                  <a
